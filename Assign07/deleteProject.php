...
<head>
	<meta charset="utf-8" />
    <title>Assign07 - Delete Project</title>
	<style type="text/css">
	...
	<fieldset>
        ...
        <ul>
            <li><label title="ProjectID" for="projectIDdis">Project ID</label>
				<span class="values"><?php if($num_result != 0){echo( trim($row["ProjectID"]));}?></span>
			</li>
            <li><label title="ProjName" for="projName">Project Name</label>
                <span class="values"><?php if($num_result != 0){echo( trim($row["ProjName"]));}?></span>
            </li>

            <li><label title="ProjCategory" for="projCategory">Category</label>
              <span class="values"><?php if($num_result != 0){echo( trim($row["ProjCategory"]));}?></span>

            </li>
			<li><label title="ProjDescription" for="projDescription">Description</label>
                <span class="values"><?php if($num_result != 0){echo( trim($row["ProjDesc"]));}?></span>
            </li>
			
              <li><label title="StartMonth" for="startMonth">Start Date</label>
                 <span class="values"><?php if($num_result != 0){echo( trim($row["StartDate"]));}?></span>
            </li>
            <li><label title="StartDay" for="startDay">End Date</label>
                 <span class="values"><?php if($num_result != 0){echo( trim($row["EndDate"]));}?></span>
            </li>
